...
initialize_submodule_repo() {
    ...
    read -p $'\nProvide a default branch name\n' -e -i "$(git config --get init.defaultBranch || echo "master")" BRANCH
    if [ $BRANCH == "" ]; then
    ...
    read -p "Please follow this link to create a !!!PRIVATE!!! repository with the correct name of $SUBMODULE_NAME"$'\n'"https://github.com/new?name=$URL_ENCODED_REPO_NAME"$'\nPress enter when ready...'
...
source $PWD/scripts/install_package.sh jq
get_submodule_name
make_new_folder
initialize_submodule_repo
